# Local Video Trajectory Drone Localizer for VS Code
# Install required packages: pip install torch torchvision opencv-python pillow faiss-cpu matplotlib numpy tkinter psutil
...
class LocalVideoTrajectoryDroneLocalizer:
    def __init__(self, base_cell_size=32, rotation_angles=[0, 45, 90, 135, 180, 225, 270, 315]):
        ...
        # Use ALL window sizes from smallest to largest for comprehensive analysis
        # Generate comprehensive range from 3x3 to 15x15
        self.window_sizes = []
        for size in range(3, 16):  # 3x3 to 15x15
            self.window_sizes.append((size, size))  # Square windows only
    ...
    def load_preprocessed_windows(self, windows_file, metadata_file):
        ...
        try:
            ...
            # Create a placeholder map image for visualization
            # This creates a light gray image with grid pattern to show structure
            self.map_image = np.ones((self.map_height, self.map_width, 3), dtype=np.uint8) * 240
            
            # Add a subtle grid pattern to make the visualization more informative
            grid_spacing = self.base_cell_size
            for i in range(0, self.map_height, grid_spacing):
                self.map_image[i:i+1, :] = [200, 200, 200]
            for j in range(0, self.map_width, grid_spacing):
                self.map_image[:, j:j+1] = [200, 200, 200]
            ...
            print(f"   📝 Using grid visualization (preprocessed windows mode)")
...
def main():
    """Main function to run the local drone localizer with all window sizes"""
    print("🚁 Local Video Trajectory Drone Localizer")
    print("=" * 60)
    ...
    print("🎯 Using all available window sizes for comprehensive analysis")
    print()
    ...
    print("\n📊 Step 3: Processing map with all window sizes...")
    ...
    try:
        ...
        if hasattr(localizer, 'current_map_path'):
        ...
        if not windows_loaded:
            print("\n🔄 Processing map with all window sizes...")
            localizer.process_map()
        else:
            print("\n✅ Using preprocessed windows (skipped processing)")
    except Exception as e:
